...
Texture2D<float4> radiosity_probe_sh_red_atlas : register(t0);
Texture2D<float4> radiosity_probe_sh_blue_atlas : register(t1);
Texture2D<float4> radiosity_probe_sh_green_atlas : register(t2);
...
FTwoBandSHVectorRGB GetRadiosityProbeSH(uint2 RadiosityProbeAtlasCoord)
{
	...
	float4 sh_green = radiosity_probe_sh_blue_atlas.Load(int3(RadiosityProbeAtlasCoord, 0));
	float4 sh_blue = radiosity_probe_sh_green_atlas.Load(int3(RadiosityProbeAtlasCoord, 0));
...
void LumenRadiosityIntegrateCS(uint3 thread_index : SV_DispatchThreadID)
{
    ...
    if(depth != 0)
    {
        float2 bilinear_weight = float2(SURFACE_CACHE_PROBE_TEXELS_SIZE * 2 - sub_tile_pos.x, SURFACE_CACHE_PROBE_TEXELS_SIZE * 2 - sub_tile_pos.y) / float2(SURFACE_CACHE_PROBE_TEXELS_SIZE, SURFACE_CACHE_PROBE_TEXELS_SIZE);
        ...
        irradiance_sh = AddSH(irradiance_sh, MulSH(sub_irradiance_sh01, weights.x));
        irradiance_sh = AddSH(irradiance_sh, MulSH(sub_irradiance_sh10, weights.x));
        irradiance_sh = AddSH(irradiance_sh, MulSH(sub_irradiance_sh11, weights.x));
        ...
        surface_cache_indirect_lighting[pixel_atlas_pos] = float4(texel_radiance.xyz * card_data.albedo, 0.0);
    }
}